...
export default () => {
  const { visible, data } = nx.$modal.value('outbound-undo');
  const { id, boxDeliveryOrderId, boxDeliveryOrderItemId } = data || {};

  const qyResource = useQuery(
    ['collection_tasks_item_sn_undo_list', id, boxDeliveryOrderId, boxDeliveryOrderItemId],
    () =>
      nx.$api.collection_tasks_item_sn_undo_list({
        id,
        boxDeliveryOrderId,
  ...
  const columns = nx.antColumn([
    { title: 'Id', key: 'id' },
    { title: 'BoxSKUCode', key: 'boxSkuCode' },
