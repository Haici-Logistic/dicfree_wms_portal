...
const ResourceTableDetail = (props) => {
  ...
  const qyResource = useQuery(
    ['product-outbounds-verify.list', 'product_delivery_orders_item_done_calc_list', ts, id],
    () => nx.$api.product_delivery_orders_item_done_calc_list({ id }),
    { enabled: !!id }
  );
  const dataSources = qyResource.data || [];
  const columns = nx.antColumn(['id', { key: 'productSkuCode' }, { title: 'Actual', key: 'totalCount' }]);
