...
export default () => {
  ...
  const state = useStateProxy({ basketNo: null, uniqueNo: null, first: '', hasError: false, id: null });
  const mtVerify = useMutation((inValues) => nx.$api.product_wave_task_sn_sorting(inValues));
  ...
  const handleFinish = async (e) => {
    const res = await nx.$api.product_wave_task_info({ uniqueNo: e.first });
    ...
    if (!id) {
      ...
      nx.$invalidQuery('product-outbounds-sorting.list');
      if (!hasError) state.basketNo = res2;
    } else {
      const { id, uniqueNo } = res;
      state.uniqueNo = uniqueNo;
      state.id = id;
    }
    reset();
  };
  ...
  return (
    <div className="border rounded">
      <Header id={state.id} label={state.uniqueNo} />
      ...
      <DataList id={state.id} basketNo={state.basketNo} />
    </div>
  );
};
