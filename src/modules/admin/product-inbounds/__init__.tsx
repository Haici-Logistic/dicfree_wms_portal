...
export default () => {
  ...
  const { id } = nx.$use('productInbound');
  const [form] = Form.useForm();
  const mainRef = useRef(null);
  const mtPrint = useMutation<any>({ mutationFn: nx.$api.product_printer_print });
  const qyAvUndoList = useQuery(
    ['product_arrival_orders_undo_list', session, id],
    () => nx.$api.product_arrival_orders_undo_list(),
    {
      // when success
      onSuccess: (data) => {
        const first = data[0];
        if (first && !id) nx.$set('productInbound.id', first.value);
      }
    }
  );
    ...
    <div className="border rounded">
      ...
      <section className="rounded-md bg-slate-100 p-5">
        <Form layout="vertical" form={form} initialValues={initialValues} onFinish={handleFinish}>
          <Form.Item name="id">
            <LgSelect
              onChange={(e) => nx.$set('productInbound.id', e.target.value)}
          ...
          <div className="gap-2">
            ...
            {state.type === 'SUPPLIER_BOX_CODE' && (
              <Form.Item name="second">
                <LgInput autoComplete="off" size="large" type="text" placeholder="Extend infomation" />
              </Form.Item>
            )}
          </div>
