...
const ResourceTableDetail = (props) => {
  ...
  const qyResource = useQuery(
    ['product-outbounds-sorting.list', 'product_wave_task_item_done_calc_list', ts, id],
    () => nx.$api.product_wave_task_item_done_calc_list({ id }),
    { enabled: !!id }
  );
  const dataSources = qyResource.data || [];
  const columns = nx.antColumn(['id', { key: 'basketNo' }, { title: 'Actual', key: 'totalCount' }]);
