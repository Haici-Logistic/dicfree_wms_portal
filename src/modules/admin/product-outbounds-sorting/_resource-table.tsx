...
const ResourceTable = (props) => {
  ...
  const qyResource = useQuery(
    ['product-outbounds-sorting.list', ts, id],
    () => nx.$api.product_wave_task_item_undo_calc_list({ id }),
    { enabled: !!id }
  );
  ...
  const columns = nx.antColumn([
    ...
    'waybill',
    {
      title: 'Undo/Plan',
      callback: (_, record) => {
        const { sortingCount, totalCount } = record;
        const left = totalCount - sortingCount;
        return (
          <span>
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Button
            onClick={() => {
              const productDeliveryOrderId = record.id;
              nx.$modal.present('product-outbound-sorting-undo', {
                id,
