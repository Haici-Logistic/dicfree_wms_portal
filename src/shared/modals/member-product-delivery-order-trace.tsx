...
export default (props) => {
  const { visible, data } = nx.$modal.value('member-product-delivery-order-trace');
  if (!data) return null;
  const id = data?.id;
  // const abortCtrl = React.useRef(new AbortController());
  const traceQuery = useQuery(
    ['member_product_delivery_orders_trace', id],
    ({ signal }) => nx.$api.member_product_delivery_orders_trace({ id }, { signal }),
    { enabled: !!id }
  );
  ...
  return (
    <Modal
      ...
      width={1000}
      footer={null}
      title={<span className="capitalize">member-product-delivery-order-trace</span>}
      onCancel={() => {
        nx.$client.cancelQueries({ queryKey: ['member_product_delivery_orders_trace'] });
        nx.npdone();
        nx.$modal.dismissAll();
      }}
      closable>
